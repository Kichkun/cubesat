...
void check(){
      ...
      Serial.print(" L2: " + String(analogRead(A0)));
      Serial.print(" L3: " + String(analogRead(A0)));
      Serial.print(" L4: " + String(analogRead(A0)));
...
void sendNames(bool li = true, bool ac = true, bool gy = false, bool ma = true, bool dh = false){
...
void sendTab(bool li = true, bool ac = true, bool gy = false, bool ma = true, bool dh = false){
      dht.read();
      String web1 = "<TABLE>";
      web1+= "<TR><TD>";
      web1+=String(schet);
...
void loop()
{
  ...
  if(Serial1.available())
  {
    ...
    if(Serial1.find("+IPD,"))
    {
     ...
     sendNames(false,true,true,true,false); // свет, акселерометр, магнитометр, гироскоп, температура и влажность
     }
    /////////////////////Sending data to browser
    for (int i=1; i<3000;i++)
     sendTab(false,true,true,true,false);
